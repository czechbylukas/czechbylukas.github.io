...
export function startGame(state) {
  ...
  function handleCheck() {
    ...
    // REMOVES SLASH: "mám rád/a" -> "mám ráda"
    const correctRaw = input.dataset.surface.replace(/\//g, '').trim();
    
    const userStripped = normalize(userRaw);
    const correctStripped = normalize(correctRaw);
    ...
    if (isCorrect) {
      ...
      if (isStrict && userRaw.toLowerCase() === correctRaw.toLowerCase()) {
        feedback.textContent = "❌ Téměř! Pozor na velká písmena nebo diakritiku.";
      } else {
        feedback.textContent = isStrict ? "❌ Špatně. Zkontrolujte detaily." : "❌ Zkuste to znovu.";
      }
      input.focus();
    }
  }
  ...
  function showQuestion() {
    if (currentIndex >= total) {
      container.innerHTML = `
        <div style="text-align:center; padding: 40px;">
          ...
          <button onclick="location.reload()" style="padding: 10px 20px; cursor:pointer; background:#2f52b5; color:white; border:none; border-radius:8px;">Hrát znovu</button>
        </div>`;
      return;
    }
    ...
    // --- GENDER DETECTION ---
    // If the Czech word contains a slash (like rád/a), show (fem.)
    const isFeminine = q.cs.includes('/');
    const genderHint = isFeminine ? ' <span style="font-size: 1rem; color: #e67e22;">(fem.)</span>' : '';
    ...
    container.innerHTML = `
      <div style="margin-bottom: 15px; background: #f4f4f4; padding: 10px; border-radius: 8px;">
        <label style="cursor: pointer; display: flex; align-items: center; gap: 10px; justify-content: center;">
          <input type="checkbox" id="strictMode"> 
          <span style="font-weight: bold;"> Strict Mode (Diakritika + Velká písmena)</span>
        </label>
      </div>
      <div style="text-align: center; padding: 20px;">
        <p style="font-size: 1.2rem; color: #666; margin-bottom: 5px;">Přeložte slovo:</p>
        <h2 style="font-size: 2.5rem; margin-bottom: 20px; color: #2c3e50;">${q.en}${genderHint}</h2>
        <div style="font-size: 2rem;">${finalHtml}</div>
        <div style="margin-top: 30px;">
           <button id="check" style="padding: 12px 30px; font-size: 1.2rem; background: #2f52b5; color: white; border: none; border-radius: 8px; cursor: pointer; font-weight: bold;">Kontrola</button>
        </div>
    ...
    input.onkeydown = (e) => { 
      if (e.key === " " || e.code === "Space") { e.stopPropagation(); return; }
